...
이에 주어진 도메인에 대한 부분집합 $A\subset X$와 확률 분포 $D$에 수 $D(A)$를 배정할 수도 있다.
$D(A)$는 $x\in A$라는 점에 대한 관찰을 신뢰할 수 있는 수준을 암시한다.  
그리고 $A$는 함수 $\pi:X\rightarrow\lbrace 0, 1 \rbrace$로 아래처럼 나타낸다.

$$
A=\lbrace x\in X:\pi(x)=1\rbrace.
$$

이에 분류기 $h:X\rightarrow Y$의 오류를 다음처럼 정의한다.
