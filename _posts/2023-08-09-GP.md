...
- 벡터공간이 $\mathbb{C}^2$라면, 생성 집합은 아래 두 벡터로 구성된다.
    ...
    \ket{v_2}\equiv\begin{bmatrix}0\\1\end{bmatrix}_.
    $$
...
- 벡터공간에는 생성 집합이 여럿 있을 수 있다. $\mathbb{C}^2$의 또 다른 
  ...
  \ket{v_2}\equiv\frac{1}{\sqrt2}\begin{bmatrix}1\\-1\end{bmatrix}_.
  $$
...
$a_i\neq 0$인 집합이 존재하여
    ...
    인 경우 **선형 종속**이다. 반대, 다시 말해, $\ket{v_1},\ldots\ket{v_n}$의 선형결합이 $0$이라면
    모든 $a_i=0$인 경우만 **선형 독립**이다.
...
둔다.
    ...
    $a=b=c=0$인 경우만 세 벡터의 선형결합이 $0$이므로, 세 벡터는 
    선형독립이다. $\square$
...
- 이는 $\ket{v_1}\equiv(1/\sqrt2)[1,],\ket{v_2} \equiv(1/\sqrt2)[1,-1]$ 같은
  생성집합을 기저로 사용할 수 있는 근거다. 
